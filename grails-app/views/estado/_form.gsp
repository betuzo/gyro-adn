<%@ page import="com.gyro.adn.domain.Estado" %>
...
<div class="fieldcontain ${hasErrors(bean: estadoInstance, field: 'pais', 'error')} required">
	...
	<g:select id="pais" name="pais.id" from="${com.gyro.adn.domain.Pais.list()}" optionKey="id" required="" value="${estadoInstance?.pais?.id}" class="many-to-one"/>
</div>
